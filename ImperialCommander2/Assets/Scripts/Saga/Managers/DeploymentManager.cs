...
namespace Saga
{
	public class DeploymentManager : MonoBehaviour
	{
		...
		void DoDeployGroup( DeploymentCard cardDescriptor, bool skipEliteModify = false )
		{
			...
			// Tokens were stacked on the hand card id before any difficulty swap
			string handCardId = cardDescriptor.id;
				 ...
				 DataStore.sagaSessionData.gameVars.availableAttachments.Count > 0 )
			{
				...
				{
					string attachmentID = DataStore.sagaSessionData.gameVars.availableAttachments[i];
					var attachment = DataStore.sagaSessionData.gameVars.attachmentDefinitions.FirstOrDefault( a => a.id == attachmentID );
